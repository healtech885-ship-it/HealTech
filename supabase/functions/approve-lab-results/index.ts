...
Deno.serve(createRoleHandler(["admin", "doctor"], async ({ body, userId, role, supabase }) => {
  ...
  const ids = Array.isArray(body.lab_result_ids) ? body.lab_result_ids : [];
  if (ids.length === 0) throw new Error("No lab results selected");

  if (role !== "admin") {
    const { data: unauthorized } = await supabase.from("lab_results").select("id,doctor_id").in("id", ids).neq("doctor_id", userId);
