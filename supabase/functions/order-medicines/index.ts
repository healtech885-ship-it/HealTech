...
Deno.serve(createRoleHandler(["admin", "doctor"], async ({ body, userId, role, supabase }) => {
  requireFields(body, ["visit_id", "items"]);
  const items = Array.isArray(body.items) ? body.items : [];
  if (items.length === 0) throw new Error("At least one medicine is required");

  const { data: visit } = await supabase.from("visits").select("id,patient_id,doctor_id,status").eq("id", body.visit_id).single();
  ...
  const { data: order, error } = await supabase.rpc("create_prescription_tx", {
    actor: userId,
